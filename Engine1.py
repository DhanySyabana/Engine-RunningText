...
def process_channel(channel: str, channel_path: str) -> bool:
    """
    Memproses satu video berikutnya untuk channel yang diberikan.
    Jika tidak ada video baru, fungsi ini langsung return False.
    Jika ada video diproses, return True.
    """
    log_id = None
    tries  = 0
    ...
    try:
        ...
        send_telegram_alert(engine_name="Engine1", channel_name=channel, error_message=str(e), stacktrace=traceback.format_exc())
...
def delete_routine() -> None:
    ...
    try:
        ...
        print(traceback.format_exc())
        send_telegram_alert(engine_name="Engine1", channel_name="DELETE_ROUTINE", error_message=str(e), stacktrace=traceback.format_exc())
...
if __name__ == '__main__':
    ...
    while True:
        ...
        for channel, channel_path in CHANNELS:
            if process_channel(channel, channel_path):
                processed_any = True
        ...
        if not processed_any:
            print(f"\n[ENGINE] Tidak ada data video baru di semua channel. Proses dihentikan.")
            break
