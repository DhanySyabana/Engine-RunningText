...
URL = "https://siputra-api.kurasi.media/streams/rumus-ad-value"
zero_value_counters = {}
...
def channelThread(channel):
     while True:
        ...
        if not data or len(data) == 0:
            print(f"No more data to process for channel: {channel}")
            break
        
        batch_size = 100
        for i in range(0, len(data), batch_size):
            batch_data = data[i:i+batch_size]
            print(f"Processing batch {i//batch_size + 1}: {len(batch_data)} items for channel: {channel}")
            calculatePriceBatched(batch_data, channel)
